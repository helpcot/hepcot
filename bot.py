...
@client.command()
async def ahelp(ctx):
    emb = discord.Embed(colour=0x1a111d, description=f"**Команды для администрации**")

    emb = discord.Embed(description= f'**Админ команды:**', color=0x6fdb9e)
    emb.add_field( name = '**Команды только для администрации**', value = '''
        /clear - Команда удаляет сообщения
    `Использование: /clear (кол-во сообщений,которое нужно удалить)`
        /kick - Команда для кика участника
    `Исполозование: /kick @Полозователь причина`
        /ban - Команда для бана участников
    `Исполозование: /ban @Полозователь причина`
        /unban - Команда для разбана участников
    `Исполозование: /unban @Полозователь причина`
        /mute - Команда для мута участников
    `Исполозование: /mute @Полозователь 1 h причина`
        /unmute - Команда для снятия мута с участника
    `Исполозование: /unmute @Полозователь`
        /ник - Команда для смена ника
    `Исполозование: /ник @Пользователь (ник,который вы хотите сделать)`
        /daun - Просто забавная команда
    `Исполозование: /daun @Полозователь`
            /crush - Просто забавная команда
    `Исполозование: /crush`
        ''' )
